<script lang="ts">
   ...
   * a sheet of nine panels becomes a scroll through everything somebody has
   * ever opened, and the state lines in the headings - the whole reason the
   * panels are folded - stop being readable at a glance. conventions.md §3.5.
   *
  ...
  /* The nine panels, by the name the list gives them. Held so that opening the
     sheet can fold them back - see openSettings() - which is the one thing
     about this column that is not the browser's own accordion. A record filled
     by `bind:this` rather than a lookup by id: the elements are this
     component's, and it is the one place that can say so without searching. */
  const folded: Record<string, HTMLDetailsElement> = $state({});

  $effect(() => {
    if (settingsOpen()) { if (!dialog.open) dialog.showModal(); }
    else if (dialog.open) dialog.close();
  });

  $effect(() => {
    foldEpoch();
    for (const one of PANELS) folded[one]?.removeAttribute("open");
    folded[OPENS_WITH]?.setAttribute("open", "");
  });
  ...
  /* A folder that was set and cannot be read is the one state worth unfolding
     ...
     and this component is what holds the element. */
  $effect(() => {
    useUnfoldSymbols(() => folded.symbolsPanel?.setAttribute("open", ""));
  });
</script>
...
<dialog bind:this={dialog} id="voices" class="sheet panels" onclose={closeSettings}>
  <div class="head">
    <strong id="settingsHeading">{t("ui.settings")}</strong>
    <button id="voiceClose" class="btn quiet icon" type="button" title={t("ui.close")} aria-label={t("ui.close")} onclick={closeSettings}>✕</button>
  </div>
  <div class="body">
  ...
  <!-- First, and deliberately: somebody who cannot read the page needs this
       ...
       for the same reason. -->
  <details bind:this={folded.languagePanel} class="panel" name="settings" id="languagePanel" open>
    <summary>
      <span class="section" id="languageSection">{t("ui.language")}</span>
      <span class="state" id="languageState">{pageLanguageName()}</span>
    </summary>
    <div class="setting">
  ...
  <details bind:this={folded.themePanel} class="panel" name="settings" id="themePanel">
    <summary>
      <span class="section" id="themeSection">{t("ui.theme")}</span>
      <span class="state" id="themeState">{themeLabel(themeNow())}</span>
    </summary>
    <div class="setting">
  ...
  <details bind:this={folded.voicesHerePanel} class="panel" name="settings" id="voicesHerePanel">
    <summary>
      <span class="section" id="voicesHereSection">{t("ui.voices_here")}</span>
      <span class="state" id="voicesHereState">{voicesHereState()}</span>
    </summary>
    <div class="setting">
  ...
  <details bind:this={folded.azurePanel} class="panel" name="settings" id="azurePanel">
    <summary>
      <span class="section" id="azureSection">{t("ui.azure")}</span>
      <span class="state" id="azureState">{azureState$()}</span>
    </summary>
    <div class="setting">
  ...
  <!-- The two symbol sources, each stating what it is. They are not exclusive:
       ...
       both licences. -->
  <details bind:this={folded.arasaacPanel} class="panel" name="settings" id="arasaacPanel">
    <summary>
      <span class="section" id="arasaacSection">{t("ui.arasaac")}</span>
      <span class="state" id="arasaacState">{activeSource() === "arasaac" ? t("ui.source_active") : t("ui.arasaac_state")}</span>
    </summary>
    <div class="setting">
  ...
  <details bind:this={folded.symbolsPanel} class="panel" name="settings" id="symbolsPanel">
    <summary>
      <span class="section" id="symbolsSection">{t("ui.symbols")}</span>
      <span class="state" id="symbolsState">{symbolsSummary()}</span>
    </summary>
    <div class="setting">
  ...
  <details bind:this={folded.boardPanel} class="panel" name="settings" id="boardPanel">
    <summary>
      <!-- No state line: this panel is two actions, and a heading that says
           nothing is furniture rather than a summary. -->
      <span class="section" id="boardSection">{t("ui.collection")}</span>
    </summary>
    <div class="setting">
  ...
  <details bind:this={folded.dataPanel} class="panel" name="settings" id="dataPanel">
    <summary>
      <span class="section" id="dataSection">{t("ui.data_section")}</span>
    </summary>
    <div class="setting">
  ...
  <details bind:this={folded.dangerPanel} class="panel" name="settings" id="dangerPanel">
    <summary>
      <span class="section" id="dangerSection">{t("ui.danger_section")}</span>
    </summary>
    <div class="setting">
