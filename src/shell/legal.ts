...
function open(page: Page): void {
  for (const id of Object.keys(PAGES) as Page[]) byId(id).hidden = id !== page;
  byId("legalHeading").textContent = t(PAGES[page]);
  ...
  byId("legal").querySelector(".body")!.scrollTop = 0;
  byId<HTMLDialogElement>("legal").showModal();
}

export function wireLegal(): void {
  byId<HTMLButtonElement>("aboutLink").onclick = () => open("aboutPage");
  byId<HTMLButtonElement>("impressumLink").onclick = () => open("impressumPage");
  byId<HTMLButtonElement>("privacyLink").onclick = () => open("privacyPage");
  byId<HTMLButtonElement>("legalClose").onclick = () =>
    byId<HTMLDialogElement>("legal").close();
}
