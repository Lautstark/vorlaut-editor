...
function run(db, names, mode, work) {
...
export function serialise(layout) {
...
export async function versionOf(text) {
...
export async function writeLayout(layout, expected) {
...
export async function recordBuild(version) {
  const db = await open();
  await run(db, [CONTENT], "readwrite", (tx) => {
...
export async function putFile(which, name, bytes) {
  const db = await open();
...
export async function getFile(which, name) {
  const db = await open();
...
export async function listFiles(which) {
  const db = await open();
  const box = await run(db, [folder(which)], "readonly", (tx, out) => {
    out.files = [];
...
export async function dropFile(which, name) {
  const db = await open();
  await run(db, [folder(which)], "readwrite", (tx) => {
...
export async function empty(which) {
  const db = await open();
  await run(db, [folder(which)], "readwrite", (tx) => {
