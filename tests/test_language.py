...
GERMAN_BY_DESIGN = {
    ...
    # firmware/vorlaut/texts.h was here, and it is the reason this file is a
    # copy rather than a move. It does not read the firmware - it allowlists it,
    # as a file permitted to hold German - and the talker keeping firmware/ with
    # no copy of this check would lose the German-word rule over exactly the
    # files the rule was written for. So both halves have this file, and this
    # half has no firmware/ for the entry to name.
    ...
    # than the commit that caused it.
    #
    # There is no CHANGELOG.md here yet - release-please stayed with the tags
    # it publishes, and this repository arrives at an empty tag namespace. The
    # entry is kept rather than removed because the argument above is about a
    # generated file and not about a repository, and a `git ls-files` sweep
    # simply never reaches a name that is not there.
    "CHANGELOG.md",
}
...
# third_party/ is a pinned checkout of another repository (see its README).
# git ls-files reports the submodule as one entry and reading it raises, so
# nothing under it is reached anyway; naming it here says so on purpose rather
# than by accident.
SKIP_PREFIX = ("content/", "example/symbols/", "static/vendor/",
               "exchange/fixtures/", "third_party/")
