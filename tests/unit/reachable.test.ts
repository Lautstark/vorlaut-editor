 ...
 * them separately would have reported every shared file twice - once as
 * reached, once as dead. adr/0012 took the loader page to the repository the
 * firmware is in, and the walk goes back to what it was. TREES stays a list
 * rather than becoming a constant, because vite.config.ts's rollupOptions
 * still names the entry points and the two lists still have to agree.
 */
 ...
 *
 * vite.config.ts's rollupOptions.input is the same fact, and it is the one
 * that decides whether a page is built at all. If a second page is ever added
 * there and not here, this test goes quiet about a whole directory - which is
 * the failure the list being short and visible is meant to make obvious. */
const TREES = [
  { dir: "src", entry: "main.ts" },
];

/** Every module in the tree, however deep, as a path from the repository
 *  root rather than from the tree - which is what let one walk cross from one
 *  tree into the other while there were two, and costs nothing now there is
 *  one. */
...
check("nothing sits in the tree that the page never loads", orphans.length === 0,
      orphans.length ? orphans.join(", ")
                     : `${all.length} modules, all reached from ${TREES.length} entry point(s)`);
